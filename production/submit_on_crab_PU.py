...
config.General.workArea = 'Tau3MuPUprofile_%s' % production_tag
...
config.JobType.psetName = '../test/makePUprofile_miniAOD.py'
  ...
  with open(args.yaml) as f:
    ...
    for sample, info in doc['samples'].items():
      ...
      for part in parts:
        ...
        config.JobType.outputFiles = ['_'.join(['tau3muPUprofile', 'mc' if isMC else 'data', production_tag])+'.root']
